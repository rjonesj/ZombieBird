...
public class ScrollHandler {
     ...
     *  - If a pipe's midpoint with respect to x is lesser than bird's beak, we are going to add 1 to our score.
        - Since we do not want this to repeat, we need to keep a boolean variable for EACH Pipe called isScored.
          Only if isScored is false, will we add one to the score (Setting isScored to true in the process.
          If we just need to change isScored back to false when the Pipe resets,
          then this will work the next time the Pipe returns).
